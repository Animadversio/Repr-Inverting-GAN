...
invert_resnet = ModulatedResNetInverse([2, 2, 3, 2], leaky_relu_rgb=True,).cuda().eval()
#%%
saveroot = r"D:\DL_Projects\Vision\Resnet_inverter"
savedir = join(saveroot, "mod_zy_inp_smlr_lr-4")
...
optim = torch.optim.Adam(invert_resnet.parameters(), lr=1e-4)
    ...
    for epoch in range(50):
        for i, (imgtsrs, _) in enumerate(tqdm(dataloader)):
            ...
            if i % 20 == 0:
                ...
                save_imgrid(((img_recon.float().detach().cpu() + 1) / 2).clamp(0, 1),
